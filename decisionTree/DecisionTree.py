...
class Leaf:
    ...
    def __init__(self, rows, id, depth):
        ...
        self.predictedLabel = lables_count(rows)
        self.id = id
        self.depth = depth
...
def print_tree(node, spacing=""):
    ...
    # Base case: we've reached a leaf
    if isinstance(node, Leaf):
        print(spacing + "Leaf with node id " + str(node.id) + " Predict " + str(
            node.predictions) + " Class = " + node.predictedLabel)
...
def computeAccuracy(rows, node):
    ...
    if count == 0:
        return 0
    for row in rows:
        # according to the dataset, label is always the last column
        label = row[-1]
